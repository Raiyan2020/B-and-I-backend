    ...
    $(document).ready(function() {
            ...
            columns: [{
                ...
                },
                {
                    ...
                    render: function(data) {
                        ...
                        if (data && data !== null && data !== '' && data !== undefined) {
                            return `<img class="table-image" src="${data}" alt="Feature Image" onerror="this.onerror=null; this.parentElement.innerHTML='<div class=\\'table-image-placeholder\\'><i class=\\'feather icon-image\\'></i></div>';" />`;
                        }
                ...
                },
                {
                    ...
                    render: function(data, type, row) {
                        let editRoute = '{{ route('admin.features.edit', ':id') }}'.replace(':id',
                            data);
                        let deleteRoute = '{{ route('admin.features.destroy', ':id') }}'.replace(
                            ':id', data);
                        let toggleStatusRoute = '{{ route('admin.features.toggleStatus', ':id') }}'
                            .replace(':id', data);
        ...
        table.one('init.dt', function() {
            if (window.TableSelection) {
                window.TableSelection.init('#features-table');
            }
        });
        ...
        if (window.BulkDelete) {
            window.BulkDelete.init({
                tableSelector: '#features-table',
                deleteUrl: '{{ route('admin.features.destroyMultiple') }}',
        ...
        $(document).on('click', '.toggle-status-btn', function(e) {
            ...
            Swal.fire({
                ...
                text: currentStatus == 1 ? '{{ __('dashboard.deactivate_feature_confirm') }}' : '{{ __('dashboard.activate_feature_confirm') }}',
                icon: 'question',
